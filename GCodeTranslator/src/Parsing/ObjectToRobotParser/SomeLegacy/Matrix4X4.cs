...
namespace GCodeTranslator.Parsing.ObjectToRobotParser.SomeLegacy;


/// <summary>
/// Класс используется только в функции CoordRotate() в парсере <see cref="ToRobotParser"/>, которая якобы не работает,
/// потому я просто скопировал целиком этот класс, не разбираясь, что тут происходит
/// </summary>
public class Matrix4X4: IEquatable<Matrix4x4>
    {
        ...
        public float this[int row, int column]
        {
            get
            {
                return (float)GetElement(row, column);
            }
        ...
        public Vector3 Position
        {
            get
            {
                return new Vector3((float)GetElement(3, 0), (float)GetElement(3, 1), (float)GetElement(3, 2));
            }

            set
            {
                SetElement(3, 0, (double)value.X);
                SetElement(3, 1, (double)value.Y);
                SetElement(3, 2, (double)value.Z);
            }
        }
        ...
        {
            AddTranslate(new Vector3((float)x, (float)y, (float)z));
        }
        ...
        public void Rotate(double AngleRadians, Vector3 Axis)
        {
            //Axis.Normalize();
        ...
        public void TransformVector(out Vector3 Changed, Vector3 Original)
        {
            ...
            Changed.X += (float)GetElement(3, 0);
            Changed.Y += (float)GetElement(3, 1);
            Changed.Z += (float)GetElement(3, 2);
        }
        ...
        {
            Changed.X = (float)(GetElement(0, 0) * Original.X + GetElement(1, 0) * Original.Y + GetElement(2, 0) * Original.Z);
            Changed.Y = (float)(GetElement(0, 1) * Original.X + GetElement(1, 1) * Original.Y + GetElement(2, 1) * Original.Z);
            Changed.Z = (float)(GetElement(0, 2) * Original.X + GetElement(1, 2) * Original.Y + GetElement(2, 2) * Original.Z);
        }
        ...
        public void GetXAxisVector(Vector3 result)
        {
            ...
            result.X = (float)GetElement(0, 0);
            result.Y = (float)GetElement(0, 1);
            result.Z = (float)GetElement(0, 2);
        }
